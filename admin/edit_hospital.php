...
$stmt = $db->prepare("SELECT * FROM Hospitals WHERE hospital_id = :hospital_id");
$stmt->execute(['hospital_id' => $hospital_id]);
$hospital = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$hospital) {
    echo "خطا: بیمارستان موردنظر یافت نشد!";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $db->prepare("UPDATE Hospitals SET name = :name, location = :location, room_price = :room_price WHERE hospital_id = :hospital_id");
        $stmt->execute([
            'name' => $name,
            'location' => $location,
...
<head>
    ...
    <title>ویرایش بیمارستان - آستور</title>
    ...
    <div class="flex flex-col min-h-screen">
        <main class="flex-1 p-4 md:p-8 pt-20">
            <div class="card p-6 max-w-2xl mx-auto">
                <h2 class="text-xl font-semibold text-white mb-4"><i class="fas fa-hospital mr-2 text-yellow-400"></i> ویرایش بیمارستان</h2>
                
                <?php if ($success): ?>
                    <div class="success text-white">بیمارستان با موفقیت ویرایش شد! <a href="/site/admin/hospitals.php" class="underline">بازگشت به لیست</a></div>
                <?php endif; ?>
                <?php if (!empty($errors)): ?>
                ...
                <form method="POST" class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="name" value="<?php echo htmlspecialchars($hospital['name']); ?>" required>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="number" name="room_price" value="<?php echo htmlspecialchars($hospital['room_price']); ?>" step="0.01">
                    </div>
