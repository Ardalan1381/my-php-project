...
if (!$doctor) {
    echo "خطا: پزشک موردنظر یافت نشد!";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $db->prepare("UPDATE Doctors SET name = :name, specialty = :specialty, availability = :availability, fee = :fee WHERE doctor_id = :doctor_id");
        $stmt->execute([
            'name' => $name,
            'specialty' => $specialty,
    ...
    <div class="flex flex-col min-h-screen">
        <main class="flex-1 p-4 md:p-8 pt-20">
            <div class="card p-6 max-w-2xl mx-auto">
                <h2 class="text-xl font-semibold text-white mb-4"><i class="fas fa-user-md mr-2 text-yellow-400"></i> ویرایش پزشک</h2>
                
                <?php if ($success): ?>
                    <div class="success text-white">پزشک با موفقیت ویرایش شد! <a href="/site/admin/doctors.php" class="underline">بازگشت به لیست</a></div>
                <?php endif; ?>
                <?php if (!empty($errors)): ?>
                ...
                <form method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="availability" value="<?php echo htmlspecialchars($doctor['availability']); ?>">
                    </div>
                    <div>
                        <label class="block text-sm text-gray-300">هزینه (تومان)</label>
                        <input type="number" name="fee" value="<?php echo htmlspecialchars($doctor['fee']); ?>" step="0.01">
                    </div>
