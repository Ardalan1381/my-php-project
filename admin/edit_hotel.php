...
$stmt = $db->prepare("SELECT hotel_id, hotel_name, hotel_owner, hotel_phone, hotel_star, hotel_province, hotel_city, hotel_publicrelations, hotel_proposed 
                      FROM Hotels WHERE hotel_id = :hotel_id");
$stmt->execute(['hotel_id' => $hotel_id]);
$hotel = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$hotel) {
    echo "خطا: هتل پیدا نشد!";
    exit;
}
    ...
    <div class="flex flex-col min-h-screen">
        ...
        <main class="flex-1 p-4 md:p-8 pt-20">
            <div class="card p-6 max-w-2xl mx-auto">
                <h2 class="text-xl font-semibold text-white mb-4"><i class="fas fa-hotel mr-2 text-yellow-400"></i> ویرایش هتل: <?php echo htmlspecialchars($hotel['hotel_name']); ?></h2>
                
                <?php if ($success): ?>
                    <div class="success text-white">هتل با موفقیت ویرایش شد! <a href="/site/admin/hotels.php" class="underline">بازگشت به لیست</a></div>
                <?php endif; ?>
                <?php if (!empty($errors)): ?>
                ...
                <form method="POST" class="space-y-4">
                    <div>
                        <label class="block text-sm text-gray-300">شناسه هتل (غیرقابل تغییر)</label>
                        <input type="text" value="<?php echo htmlspecialchars($hotel['hotel_id']); ?>" disabled>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="hotel_owner" value="<?php echo htmlspecialchars($hotel['hotel_owner'] ?? ''); ?>">
                    </div>
                    <div>
                        <label class="block text-sm text-gray-300">شماره تماس (اختیاری)</label>
                        <input type="text" name="hotel_phone" value="<?php echo htmlspecialchars($hotel['hotel_phone'] ?? ''); ?>">
                    </div>
                    <div>
                        <label class="block text-sm text-gray-300">ستاره (0-5، اختیاری)</label>
                        <input type="number" name="hotel_star" value="<?php echo htmlspecialchars($hotel['hotel_star'] ?? ''); ?>" min="0" max="5">
                    </div>
                    <div>
                        <label class="block text-sm text-gray-300">استان</label>
                        <input type="text" name="hotel_province" value="<?php echo htmlspecialchars($hotel['hotel_province']); ?>" required>
                    </div>
                    <div>
                        <label class="block text-sm text-gray-300">شهر</label>
                        <input type="text" name="hotel_city" value="<?php echo htmlspecialchars($hotel['hotel_city']); ?>" required>
                    </div>
                    <div>
                        <label class="block text-sm text-gray-300">روابط عمومی (اختیاری)</label>
                        <input type="text" name="hotel_publicrelations" value="<?php echo htmlspecialchars($hotel['hotel_publicrelations'] ?? ''); ?>">
                    </div>
                    <div>
